...
; RUN: llc -O0 -mtriple=mipsel-unknown-linux-gnu -mcpu=mips32r2 -target-abi=o32 < %s -filetype=asm -o - -rafast-ignore-missing-defs \
; RUN:   | FileCheck -check-prefixes=O32 %s
; RUN: llc -O0 -mtriple=mips64el-unknown-linux-gnu -mcpu=mips64r2 -target-abi=n32 < %s -filetype=asm -o - -rafast-ignore-missing-defs \
; RUN:   | FileCheck  -check-prefixes=N32,ALL %s
; RUN: llc -O0 -mtriple=mips64el-unknown-linux-gnu -mcpu=mips64r2 -target-abi=n64 < %s -filetype=asm -o - -rafast-ignore-missing-defs \
...
; O32-NEXT:    lw $1, 4($sp) # 4-byte Folded Reload
; O32-NEXT:    # kill: def $v0 killed $a1
; O32-NEXT:    # kill: def $v0 killed $a0
; O32-NEXT:    lui $2, %hi(sym)
; O32-NEXT:    lw $3, %lo(sym)($2)
; O32-NEXT:    sync
...
; N32-NEXT:    lw $1, 12($sp) # 4-byte Folded Reload
; N32-NEXT:    move $2, $5
; N32-NEXT:    sll $5, $2, 0
; N32-NEXT:    move $2, $4
; N32-NEXT:    sll $4, $2, 0
; N32-NEXT:    lui $2, %hi(sym)
; N32-NEXT:    lw $3, %lo(sym)($2)
; N32-NEXT:    sync
...
; N64-NEXT:    lw $1, 12($sp) # 4-byte Folded Reload
; N64-NEXT:    move $2, $5
; N64-NEXT:    sll $5, $2, 0
; N64-NEXT:    move $2, $4
; N64-NEXT:    sll $4, $2, 0
; N64-NEXT:    lui $2, %highest(sym)
; N64-NEXT:    daddiu $2, $2, %higher(sym)
; N64-NEXT:    dsll $2, $2, 16
; N64-NEXT:    daddiu $2, $2, %hi(sym)
; N64-NEXT:    dsll $2, $2, 16
; N64-NEXT:    ld $3, %lo(sym)($2)
; N64-NEXT:    sync

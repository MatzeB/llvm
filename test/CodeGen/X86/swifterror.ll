...
; CHECK-O0: movl $16
; CHECK-O0: malloc
; CHECK-O0: movq %rax, [[REG0:%[a-z]+]]
; CHECK-O0: movb $1, 8([[REG0]])
; CHECK-O0: movq %{{.*}}, %r12
entry:
...
; CHECK-O0-DAG: movb $1, 8([[ID]])
; CHECK-O0-DAG: movq %rax, %r12
; CHECK-O0: ret
; reload from stack
...
; spill sret to stack
; CHECK-O0: movq %rdi, [[ID0:%[a-z]+]]
; CHECK-O0: movq [[ID0]], [[OFFSET:[0-9]*\(%rsp\)]]
; CHECK-O0: malloc
; CHECK-O0-DAG: movq %rax, [[ID:%[a-z]+]]
; CHECK-O0-DAG: movb $1, 8([[ID]])
; CHECK-O0-DAG: movl %{{.*}}, 4(%{{.*}})
; CHECK-O0-DAG: movq %{{.*}}, %r12
; reload sret from stack
; CHECK-O0-DAG: movq [[OFFSET]], [[ID2:%[a-z]+]]
; CHECK-O0: movq [[ID2]], %rax
; CHECK-O0: ret
entry:
  %call = call i8* @malloc(i64 16)
  %call.0 = bitcast i8* %call to %swift_error*
...
; CHECK-O0: xorl
; CHECK-O0-DAG: movl {{.*}}, %r12d
; CHECK-O0-DAG: movl $1, %esi
; CHECK-O0-DAG: leaq {{.*}}, %rdi
